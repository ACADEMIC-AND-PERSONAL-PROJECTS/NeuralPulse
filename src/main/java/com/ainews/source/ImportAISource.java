...
@Component
public class ImportAISource extends AbstractHtmlSource {

    private static final String BASE_URL = "https://importai.substack.com";
    private static final String RSS_URL = BASE_URL;
    ...
    public List<NewsItem> fetchLastWeek() {
        ...
        Document doc = fetchDocument(RSS_URL);
        if (doc == null) return items;

        Elements articles = doc.select("article, div[class*='post'], a[href*='/p/']");
        for (Element el : articles) {
            try {
                ...
                log.debug("Error parsing Import AI article: {}", e.getMessage());
            }
        }
        return items;
    }
