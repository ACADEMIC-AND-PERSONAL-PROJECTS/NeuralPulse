...
@Component
public class TLDRAISource extends AbstractHtmlSource {

    private static final String BASE_URL = "https://tldr.tech";
    private static final String AI_URL = BASE_URL + "/ai";
    ...
    public List<NewsItem> fetchLastWeek() {
        ...
        Document doc = fetchDocument(AI_URL);
        if (doc == null) return items;

        Elements articles = doc.select("article, div[class*='post'], div[class*='newsletter'], a[href*='/ai/']");
        for (Element el : articles) {
            try {
                ...
                log.debug("Error parsing TLDR AI article: {}", e.getMessage());
            }
        }
        return items;
    }
