...
@Component
public class OpenAISource extends AbstractHtmlSource {

    private static final String BASE_URL = "https://openai.com";
    ...
    public List<NewsItem> fetchLastWeek() {
        ...
        Elements articles = doc.select("article, div[class*='post'], div[class*='blog'], a[href*='/blog/']");
        for (Element el : articles) {
            try {
                ...
                log.debug("Error parsing OpenAI article: {}", e.getMessage());
            }
        }
        return items;
    }
